...
export const useCheck = (props, panelState, emit) => {
  ...
  const handleAllCheckChange = (value) => {
    panelState.check = value ? checkableData.value.map(item => item[keyProp.value]) : []
  }
  ...
  const updateAllChecked = () => {
    ...
    panelState.allChecked = checkableDataKeys.length && checkableDataKeys.every(item => props.checked.includes(item))
  }

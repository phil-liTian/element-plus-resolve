<template>
  <div class="lt-transfer-panel">
    <p class="lt-transfer-panel__header">
      <lt-checkbox v-model="allChecked" @change="handleAllCheckChange"> List<span>0/0</span> </lt-checkbox>
    </p>

    <div class="lt-transfer-panel__body">
      <!-- 筛选 -->
      <lt-input v-model="query" class="lt-transfer-panel__filter" :placeholder="placeholder" v-if="filterable" @mouseenter="inputHover = true" @mouseleave="inputHover = false">
        <template #prefix>
          <i :class="['el-input__icon', `el-icon-${inputIcon}`]" 
             @click="clearQuery">
          </i>
        </template>
      </lt-input>
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const { keyProp, labelProp, filteredData, handleAllCheckChange } = useCheck(props, panelState, emit);

    const inputIcon = computed(() => {
      return panelState.query.length && panelState.inputHover ? 'circle-close' : 'search'
    })
